...
### Changed
- **`dum-solution-design` 从"5 模块固定结构"改为"基础骨架 + 按方案类型追加章节"**：
  目标改为把方案核心要点讲清楚；骨架新增开头「概要」段（5 行内讲清方案、问题、影响范围、关键决策）
  和「待决策」段（需用户拍板的开放问题，审核通过前必须清空，与「遗留问题」分开）；
  骨架段可合并或标注不适用（概要、关键逻辑、待决策、遗留问题不可省）；
  允许写类名/接口名/方法签名/字段名作代码骨架参考，只禁实现代码；
  新增新功能 / 基础架构选型 / bug 修复 / 重构迁移 / 性能优化 五类方案的追加章节表；
  根因已定位的 bug 修复方案纳入适用范围。`SKILL.md` 精简约一半篇幅。
- `dum-doc-reconcile` 对方案文档的约束由"保 5 模块结构"改为"保原有章节结构"。
- **`dum-merge-code` 验证阶段改为按"责任变更集"决定测试范围**：只有实际解决了源代码冲突才自动跑
  与冲突直接相关的最小单元测试；无冲突合并、仅文档/配置/锁文件冲突不自动测试；集成 / E2E / 全量回归等
  非单元测试执行前必须说明范围与成本并询问；验证不得把副作用带入提交。
- 各 manifest 与 `package.json` 版本 `1.4.0` → `1.5.0`。
