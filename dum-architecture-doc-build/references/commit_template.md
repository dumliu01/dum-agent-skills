...
## Commit 1：架构文档骨架 + 知识库索引

```
docs(architecture): add {{service-1}} + {{service-2}} arch docs + KB index

- docs/architecture/{{service-1}}.md (N lines): {{进程模型}} + {{核心模块}}
  + {{对外接口}} + {{持久化}} + {{测试}} + {{构建}} + {{依赖图}} +
  {{N 条 invariant}}
- docs/architecture/{{service-2}}.md (M lines): {{...}}
- docs/architecture/README.md: knowledge-base index — service doc links
  + curated must-read nav + DOC-MAP region (doc map across all dirs)
...
- `docs/architecture/README.md`（知识库索引页）
...
- scripts/update_architecture_{{service-2}}.py: walks {{...}}; extracts
  {{...}} from each {{...}} (M files).
- scripts/update_doc_map.py: project-wide doc map — walks all .md
  across the repo (excludes node_modules/.git/vendored); extracts
  frontmatter title / first H1 / first line; writes between DOC-MAP
  markers in docs/architecture/README.md (P docs indexed). Title
  changes DO refresh (unlike source manifests).
- Source scripts: silent no-op when AUTO-GENERATED markers absent;
...
- `scripts/update_doc_map.py`
- `docs/architecture/<service>.md` × N + `docs/architecture/README.md`（自动生成区填好后会再 diff）
- `.claude/settings.local.json`

## 如果一次性提交（小项目）

```
docs(architecture): {{service-1}} + {{service-2}} arch docs + KB index + auto-update hook
...
- docs/architecture/README.md: knowledge-base index + auto doc map
...
- scripts/update_doc_map.py: scan all .md, extract title, write between
  DOC-MAP markers (project-wide doc inventory)
...
.md → doc map updates; rm + manual rerun → both trees cleanly remove it.
```
