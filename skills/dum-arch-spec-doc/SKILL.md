...
- **每个展示目录都有一行职责**：说明它承载的业务域或架构层，例如「外部事件认证、幂等收据、订阅路由和等待 Run 恢复」，不能只把目录名翻译一遍。
- **每个展示文件都有一行职责**：从业务功能、架构角色、关键状态或安全边界解释该文件为什么存在，例如「提供主体能力总览，以及 Skill 安装、MCP 发布和工具配置接口」。
...
重新生成后至少检查：生产文件说明均非空、目录说明均非空、测试文件引用为 0、低信息模式为 0，并再次运行生成器确认内容和时间戳幂等稳定。
...
4. ✅ 自动源码树只含生产源码；每个展示目录和文件均有业务/架构职责说明，无符号清单或文件名复述
