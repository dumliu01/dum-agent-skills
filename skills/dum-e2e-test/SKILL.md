---
name: dum-e2e-test
description: Use when the user asks 给某功能/服务做端到端测试、e2e 测试、UI 自动化测试、生成测试用例并跑、验证带后端的前端/客户端（网页/Electron/Flutter；Android/iOS 占位）行为是否符合需求。能力：按权威阶梯(需求>技术方案>代码>用例)从文档派生用例 → 生成脚本(Web/Electron 走 Playwright，Flutter 走 integration_test) → agent 实测 → 三层校验(交互/界面数据展示/持久化) → 失败三段定责(脚本/用例/代码，并分前端/后端) → 仅出修复方案需用户确认。Triggers - "做端到端测试"/"写自动化测试"/"e2e 测试"/"生成测试用例并运行"/"验证XX功能数据是否正确"/"UI 自动化"。
---
...
v1 范围：Web/Electron（Playwright）与 Flutter（integration_test 白盒）做实，Android/iOS 以统一驱动契约占位；**全程仅出方案**，实际改动须用户确认后再执行。
...
| Android/iOS（原生壳）要跑 | v1 未支持这两端；运行会抛 `not-implemented`，勿跑（Flutter 已支持） |
...
1. **确认测试范围**：询问用户目标功能/服务，明确平台（Web/Electron/Flutter 实测；Android/iOS 占位）。
...
1. **选平台适配器**（查 `references/platform-adapters.md §2 适配器登记`）：
   ...
   | Flutter | `FlutterAdapter`（integration_test 白盒） | 是 |
   | Android / iOS | 占位适配器 | 否（抛 `not-implemented`） |
...
3. **翻译三层断言**：
   ...
   - **Flutter 差异**：① 交互/② 展示同样走 `locate`(Finder：`ValueKey`/`Semantics`)+`readDisplay`(读 widget 属性)；③ 持久化**走 API 黑盒为主**——设备/模拟器难直连测试库，DB 白盒需宿主侧 runner（见 `references/platform-adapters.md §3`）。

4. **走后门 seed**：在 `beforeEach` 中调 `gateway.seed()` 造前置数据，`afterEach` 调 `gateway.rollback(handle)` 精确回收。

5. **写脚本**：脚本目录 `tests/e2e/<feature>/`，骨架参考 `assets/playwright-adapter-skeleton.ts`（Web/Electron）或 `assets/flutter-adapter-skeleton.dart`（Flutter，integration_test）——均为参考模板，落地按目标项目调整。
...
| e2e 脚本 | `tests/e2e/<feature>/` | TypeScript（Web/Electron · Playwright）/ Dart（Flutter · integration_test），按对应骨架落地 |
...
| v1 平台 | Web / Electron（Playwright）/ Flutter（integration_test）实测；Android/iOS 占位 |
...
| Android/iOS 当作 v1 已实现去跑 | 占位适配器抛 `not-implemented`，造成混乱 | v1 跑 Web/Electron/Flutter；Android/iOS 待后续版本 |
...
- 在 v1 跑 Android/iOS 占位平台的测试 → 停，告知用户 v1 仅支持 Web/Electron/Flutter（Android/iOS 抛 `not-implemented`）
