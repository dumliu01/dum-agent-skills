...
2. **后续校对设计文档**（由 [`dum-doc-reconcile`](../dum-doc-reconcile/) 被显式触发时读取本记录完成）：本技能**不主动 grep 设计文档、不列"待校对清单"、不给修改建议、不修任何 `docs/architecture/` 或 `docs/tech-design/` 下的文件**
...
| 不做 | 不写"受影响的设计文档（待校对）"一节；不去修 `docs/architecture/` 或 `docs/tech-design/`；不顺手触发 `dum-doc-reconcile` |

## 跟其它技能怎么衔接

- **上游**接代码改动；**下游**接 [`dum-doc-reconcile`](../dum-doc-reconcile/)——后者**在被用户显式触发**时读这些记录去对账 `docs/architecture/` 与 `docs/tech-design/`。本技能只产出原材料，不替它启动。
